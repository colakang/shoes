			...
			try:
				...
				for option in skuList.get('productSkus'):
					...
					if self.skuid == option.get('sizeDescription'):
						...
						result['lang_locale'] = 'ja_JP'
						result['country'] = 'JP'

						result['catalogId'] = Json.get('catalogId')
					 	result['productId'] = Json.get('productId')
						result['price'] = Json.get('rawPrice')
						result['line1'] = urllib.quote_plus(str(json.dumps(Json.get('productTitle'),indent=2, ensure_ascii=False)))
						try:
							result['line2'] = urllib.quote_plus(str(json.dumps(Json.get('productSubTitle'),indent=2, ensure_ascii=False)))
						except:
							result['line2'] = str(json.dumps(Json.get('productSubTitle'),indent=2, ensure_ascii=False))
...
class Login_In:
	def saveCookies (self,uName,uPass):
		...
		self.url = "https://www.nike.com/profile/login?Content-Locale=ja_JP"
		...
		self.hdr = {
				...
				'Authorization' : 'CPC 1111E',
				'Connection' : 'keep-alive',
				'Content-Length' : '65',
				'Content-Locale' : 'ja_JP',
				'DNT' : '1',
				'Referer' : 'http://store.nike.com/jp/ja_jp/?sortOrder=publishdate|desc&ipp=120',
			}
		...
		opener = urllib2.build_opener(self.cookie_support,httpHandler,httpsHandler)
		...
		file_object.close( )
		#print (self.cookie)
		cs = self.parse("NIKE_COMMERCE_COUNTRY=JP; NIKE_COMMERCE_LANG_LOCALE=ja_JP; mt.m=%7B%22membership%22%3A%5B%22us_aa-el1-ae%22%5D%7D; CONSUMERCHOICE_SESSION=t; CONSUMERCHOICE=jp/ja_jp; nike_locale=ja/ja_jp; cookies.js=1;",".nike.com")
		for c in cs:
			self.cookie.set_cookie(c)
		req_test = urllib2.Request('https://secure-store.nike.com/jp/checkout/html/cart.jsp')
		req_test.add_header('Referer', 'http://store.nike.com/jp/ja_jp/?ipp=120')
		req_test.add_header('User-Agent','Mozilla/5.0')
		req_test.add_header('Accept','text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8')
		req_test.add_header('Upgrade-Insecure-Requests','1')
		req_test.add_header('Connection','keep-alive')
		req_test.add_header('Cache-Control','max-age=0')
		req_test.add_header('Accept-Encoding','gzip, deflate, sdch')
		req_test.add_header('Accept-Language','zh-CN,zh;q=0.8')
		try:
			req2 = urllib2.urlopen(req_test)
...
if __name__ == '__main__':
	...
	if Mode == "2":
		...
		nikeUrl = 'https://secure-store.nike.com/ap/services/jcartService?callback=nike_Cart_handleJCartResponse&'+add2CartUrl
	elif Mode == "1":
		fastMode = FastMode()
	...
	opener = urllib2.build_opener(cookieProc)
	urllib2.install_opener(opener)
	req_profile = urllib2.Request('https://secure-store.nike.com/jp/checkout/html/cart.jsp')
	req_profile.add_header('Referer', 'http://store.nike.com/jp/ja_jp/?ipp=120')
	#req_profile.add_header('User-agent', 'Mozilla/5.0')
	#print cookie
	cookie_check = True
	...
	if (startTime - time.time()) > 0:
		...
		time.sleep(startTime - time.time() - 0.08)
	else:
		print 'start now'
	...
	while True:
		...
		req_add2cart.add_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.3; Win64; x64)  AppleWebKit/537.36 (KHTML, like Gecko) Safari/537.36')
		try:
			...
			if html.find('success') == -1:
				if html.find('failure') == -1:
					...
					file_object.close( )
					mail = sendMail()
					mail.send_email(uName,html,"NikeJP")
					break
			else:
