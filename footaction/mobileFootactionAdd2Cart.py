...
class FastMode:
	...
	def getUrlInfo (self,refUrl,refSkuid,Mode):
		...
		req_getInfo.add_header('Referer', 'http://m.footaction.com')
...
class Login_In:
	def __init__ (self):
		self.cookiefile = "./log/"+uName+"_MfootactionCookies.txt" 
		...
		urllib2.install_opener(opener)
		req = urllib2.Request('https://m.footaction.com/?uri=account')
		req.add_header('Referer','https://m.footaction.com/')
	...
	def saveCookies (self,uName,uPass):
		...
		self.hdr = {
					...
					'Referer' : 'https://m.footaction.com/?uri=account',
		...
		self.url = "https://m.footaction.com/?uri=account/accountSignIn"
		...
		while url_check == True:
			try:
				...
				if re.search('Log Out',self.result) == None:
					...
					url_check = False
					file_object = open('./log/'+self.login_email+'_MfootactionLogin.txt', 'w')
...
if __name__ == '__main__':
	...
	nikeUrl = 'http://m.footaction.com/?uri=add2cart&fragment=true'
	try:
			...
			if html.find('"success":true') == -1:
				if html.find('Unauthorized order') != -1:
					...
					try:
						...
						req_add2cart_body = urllib.urlencode(result)
						file_object = open('./log/'+Pid+'_'+uName+'_MfootactionDebug.txt', 'w')
						file_object.write(html)
						file_object.close( )
				elif html.find('it is out of stock') != -1:
					print "SOLD OUT !!"
					file_object = open('./log/'+Pid+'_'+uName+'_MfootactionError.txt', 'w')
					file_object.write(html)
					file_object.close( )
					break
				else:
				...
				file_object = open('./log/'+Pid+'_'+uName+'_MfootactionItem.txt', 'w')
				file_object.write(html)
				file_object.close( )
				print "Check Out Now!!"
				mail = sendMail()
				mail.send_email(uName,html,"Footaction")
