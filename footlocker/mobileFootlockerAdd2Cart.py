...
class FastMode:
	...
	def getUrlInfo (self,refUrl,refSkuid,Mode):
		cookiePath = "./log/"+uName+"_MfootlockerCookies.txt" 
...
class Login_In:
	def __init__ (self):
		self.cookiefile = "./log/"+uName+"_MfootlockerCookies.txt" 
	...
	def saveCookies (self,uName,uPass):
		...
		self.url = "https://m.footlocker.com/?uri=account/accountSignIn"
		...
		while url_check == True:
			try:
				...
				if re.search('Log Out',self.result) == None:
					...
					url_check = False
					file_object = open('./log/'+self.login_email+'_MfootlockerLogin.txt', 'w')
...
if __name__ == '__main__':
	...
	nikeUrl = 'http://m.footlocker.com/?uri=add2cart&fragment=true'
	try:
			...
			if html.find('"success":true') == -1:
				if html.find('Unauthorized order') != -1:
					...
					try:
						...
						req_add2cart_body = urllib.urlencode(result)
						file_object = open('./log/'+Pid+'_'+uName+'_MfootlockerDebug.txt', 'w')
				...
				print "Check Out Now!!"
				mail = sendMail()
				mail.send_email(uName,html,"Footlocker")
				...
				pool.join()
				break
	# checkout url = http://m.footlocker.com/?uri=cart&action=checkout
