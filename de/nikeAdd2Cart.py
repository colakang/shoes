...
class Login_In:
	def saveCookies (self,uName,uPass):
		...
		self.url = "https://www.nike.com/profile/login?Content-Locale=de_DE"
		...
		file_object.close( )
		#print (self.cookie)
		cs = self.parse("NIKE_COMMERCE_COUNTRY=DE; NIKE_COMMERCE_LANG_LOCALE=de_DE; mt.m=%7B%22membership%22%3A%5B%22us_aa-el1-ae%22%5D%7D; CONSUMERCHOICE_SESSION=t; CONSUMERCHOICE=de/de_de; nike_locale=de/de_de; cookies.js=1;",".nike.com")
		for c in cs:
			self.cookie.set_cookie(c)
		req_test = urllib2.Request('https://secure-store.nike.com/de/checkout/html/cart.jsp')
		req_test.add_header('Referer', 'http://store.nike.com/de/de_de/?ipp=120')
...
if __name__ == '__main__':
	...
	opener = urllib2.build_opener(cookieProc)
	urllib2.install_opener(opener)
	req_profile = urllib2.Request('https://secure-store.nike.com/de/checkout/html/cart.jsp')
	req_profile.add_header('Referer', 'http://store.nike.com/de/de_de/?ipp=120')
		...
		try:
			...
			if html.find('success') == -1:
				if html.find('failure') == -1:
					...
					file_object.close( )
					mail = sendMail()
					mail.send_email(uName,html,"NikeDE")
					break
			else:
