...
class FastMode:
	...
	def getUrlInfo (self,refUrl,refSkuid,Mode):
		cookiePath = "./log/"+uName+"_MeastbayCookies.txt" 
		...
		req_getInfo.add_header('Referer', 'http://m.eastbay.com')
...
class Login_In:
	def __init__ (self):
		self.cookiefile = "./log/"+uName+"_MeastbayCookies.txt" 
		...
		urllib2.install_opener(opener)
		req = urllib2.Request('https://m.eastbay.com/?uri=account')
		req.add_header('Referer','https://m.eastbay.com/')
	...
	def saveCookies (self,uName,uPass):
		...
		self.hdr = {
					...
					'Referer' : 'https://m.eastbay.com/?uri=account',
		...
		self.url = "https://m.eastbay.com/?uri=account/accountSignIn"
		...
		while url_check == True:
			try:
				...
				if re.search('Log Out',self.result) == None:
					...
					url_check = False
					file_object = open('./log/'+self.login_email+'_MeastbayLogin.txt', 'w')
...
def removeCart(refUrl,uName,postData):
	...
	urllib2.install_opener(opener)
	print postData
	req_getInfo = urllib2.Request('http://m.eastbay.com/?uri=cart',postData)
	req_getInfo.add_header('Referer', 'http://m.eastbay.com/?uri=cart')
	req_getInfo.add_header('User-agent', 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36')
	req_getInfo.add_header('Accept','text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8')
	req_getInfo.add_header('Accept-Encoding','gzip, deflate, sdch')
	req_getInfo.add_header('Accept-Language','zh-CN,zh;q=0.8')
...
def checkCart(refUrl,uName,sh):
	...
	opener = urllib2.build_opener(cookieProc)
	urllib2.install_opener(opener)
	req_getInfo = urllib2.Request('http://m.eastbay.com/?uri=cart')
	req_getInfo.add_header('Referer', 'http://m.eastbay.com/?uri=cart')
	req_getInfo.add_header('User-agent', 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36')
	req_getInfo.add_header('Accept','text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8')
	req_getInfo.add_header('Accept-Encoding','gzip, deflate, sdch')
...
if __name__ == '__main__':
	...
	nikeUrl = 'http://m.eastbay.com/?uri=add2cart&fragment=true'
	try:
			...
			if html.find('"success":true') == -1:
				if html.find('Unauthorized order') != -1:
					...
					try:
						...
						req_add2cart_body = urllib.urlencode(result)
						file_object = open('./log/'+Pid+'_'+uName+'_MeastbayDebug.txt', 'w')
						file_object.write(html)
						file_object.close( )
				elif html.find('it is out of stock') != -1:
					print "SOLD OUT !!"
					file_object = open('./log/'+Pid+'_'+uName+'_MeastbayError.txt', 'w')
					file_object.write(html)
					file_object.close( )
					break
				else:
				...
				file_object = open('./log/'+Pid+'_'+uName+'_MeastbayItem.txt', 'w')
				file_object.write(html)
				file_object.close( )
				print "Check Out Now!!"
				mail = sendMail()
				mail.send_email(uName,html,"Eastbay")
				...
				pool.join()
				break
	# checkout url = http://m.eastbay.com/?uri=cart&action=checkout
