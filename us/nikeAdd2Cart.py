...
if __name__ == '__main__':
	...
	opener = urllib2.build_opener(cookieProc)
	urllib2.install_opener(opener)
	req_profile = urllib2.Request('https://secure-store.nike.com/us/checkout/html/cart.jsp')
	req_profile.add_header('Referer', 'http://store.nike.com/us/en_us/?ipp=120')
		...
		try:
			...
			if html.find('success') == -1:
				if html.find('failure') == -1:
					...
					file_object.close( )
					mail = sendMail()
					mail.send_email(uName,html,"NikeUS")
					break
			else:
