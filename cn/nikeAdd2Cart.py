...
class Login_In:
	def saveCookies (self,uName,uPass):
		...
		self.url = "https://www.nike.com/profile/login?Content-Locale=zh_CN"
		...
		file_object.close( )
		#print (self.cookie)
		cs = self.parse("NIKE_COMMERCE_COUNTRY=CN; NIKE_COMMERCE_LANG_LOCALE=zh_CN; mt.m=%7B%22membership%22%3A%5B%22us_aa-el1-ae%22%5D%7D; CONSUMERCHOICE_SESSION=t; CONSUMERCHOICE=cn/zh_cn; nike_locale=cn/zh_cn; cookies.js=1;",".nike.com")
		for c in cs:
			self.cookie.set_cookie(c)
		req_test = urllib2.Request('https://secure-store.nike.com/cn/checkout/html/cart.jsp')
		req_test.add_header('Referer', 'http://store.nike.com/cn/zh_cn/?ipp=120')
...
if __name__ == '__main__':
	...
	opener = urllib2.build_opener(cookieProc)
	urllib2.install_opener(opener)
	req_profile = urllib2.Request('https://secure-store.nike.com/cn/checkout/html/cart.jsp')
	req_profile.add_header('Referer', 'http://store.nike.com/cn/zh_cn/?ipp=120')
		...
		try:
			...
			if html.find('success') == -1:
				if html.find('failure') == -1:
					...
					file_object.close( )
					mail = sendMail()
					mail.send_email(uName,html,'NikeCN')
					break
			else:
